...
from .forms import BBCodeContentForm
from django.shortcuts import render
from .models import BBCodeContent

def bbcode_success(request):
    bbcode_content = BBCodeContent.objects.latest('id')
    return render(request, 'bbcode_success.html', {'bbcode_content': bbcode_content})

def bbcode_input(request):
    if request.method == 'POST':
        form = BBCodeContentForm(request.POST)
        if form.is_valid():
            form.save()
            return redirect('bboard:bbcode_success')
    else:
        form = BBCodeContentForm()

    return render(request, 'bbcode_input.html', {'form': form})
